...
export function getPartitionKey(eventId, paritionCount) {
  ...
  const partition = getPartition(eventId, parseInt(paritionCount, 10));

  return `${indexDateStr}.p${partition}`;
}

export function parsePartitionKey(partitionKey) {
  const [indexDateStr, partition] = partitionKey.split('.p');
  return {
    indexDateStr,
    partition,
  };
...
export function zipPartitionKeys(indexDateStr, exclusiveStartKeys) {
  return Object.entries(exclusiveStartKeys).reduce((acc, [partition, sortKey]) => {
    acc[`${indexDateStr}.p${partition}`] = sortKey;
    return acc;
  }, {});
}

export function getAllPartitionKeys(indexDateStr, paritionCount) {
  return Array(parseInt(paritionCount, 10)).fill().map((_, partition) => `${indexDateStr}.p${partition}`);
}
