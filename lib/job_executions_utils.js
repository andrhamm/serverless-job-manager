export function getIndexDateFields (date, dayOffset) {
  ...
  const indexStartMs = Date.parse(indexDate);

  return [indexDate.replace(/-/g, ''), indexStartMs, dt];
}
...
export function getIndexDate (date) {
  const [indexDate] = getIndexDateFields(date)
...
export function getAllPartitionKeys (indexDate, paritionCount) {
  return Array(parseInt(paritionCount)).fill().map((_, partition) => `${indexDate}.p${partition}`);
}
